...
%     
%     if exist([workrootdir,[socatversion,'_synth.mat']],'file')
%         load([workrootdir,[socatversion,'_synth.mat']]);
%     else % read file
...
% end
%         save([workrootdir,[socatversion,'_synth.mat']], 'SOCAT');
%     end
% end
%%
% Read SOCATv2109 enhanced files (A-E, WOCE 2-4) (not generated anymore
% from v2020 (synthesis has the extra columns of the enhanced)
if strcmp(whichsocat,'all') | strcmp(whichsocat,'both')
    if exist([workrootdir,[socatversion,'.mat']],'file')
        load([workrootdir,[socatversion,'.mat']]);
    else % read files
        allcruisefiles=dir([workrootdir,'SOCATv7All_ABCDE_enhanced_datafiles/**/*.tsv']);
        for acf=1:length(allcruisefiles);
            ...
            if acf==1; SOCATall=cruisedata;
            else SOCATall=CatStructFields(SOCATall,cruisedata,1);
            end
            clear cruisedata
        end
        save([workrootdir,[socatversion,'all.mat']], 'SOCATall', '-v7.3');
    end
end
...
for ec=10:20%length(unique_platform_code)
    ...
    switch source_platform_category_code
        ...
        case '41'
            ...
            outputfile = ['MOORING/GL_TS_MO_',platform_code,'-SOCATv2020.nc'];
        ...
        case '42'
            ...
            data_type='OceanSITES trajectory data';
            cdm_data_type='trajectory';
            outputfile = ['DRIFTER/GL_TS_DB_',platform_code,'-SOCATv2020.nc'];
    ...
    netcdf.putAtt(ncid, tempvar, 'sensor_depthh', '');
    ...
    netcdf.putAtt(ncid, globid, 'title', 'Global Ocean - In Situ reprocessed carbon observations - SOCATv2020');
...
%    netcdf.putAtt(ncid, globid, 'PI_institution', pi_institution);
    ...
    clearvars '-except' workrootdir outdir SOCAT SOCAT_info ...
