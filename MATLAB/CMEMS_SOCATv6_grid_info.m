...
typegrids={'decade','year','month','coastal'}


%workdir=['/Users/rpr061/Dropbox/BCDC_Projects/CMEMS_INSTAC/REP_Products/'...
%    '2018/SOCATv6/workspace/'];
%cd(workdir)
typegrid = typegrids{tg};%'coastal';
outputfile=[workdir,'SOCATv6_gridded_',typegrid,'_Copernicus_test.nc'];
...
switch typegrid
    case 'decade'
        inputfile = [inputdir,...
            'SOCATv6_tracks_gridded_decades.nc'];
        timevarname = 'tdecade';
        varappend = '_decade';
    ...
    case 'year'
        inputfile = [inputdir,...
            'SOCATv6_tracks_gridded_yearly.nc'];
        timevarname = 'tyear';
        varappend = '_year'
    ...
    case 'month'
        inputfile = [inputdir,...
            'SOCATv6_tracks_gridded_monthly.nc'];
        timevarname = 'tmnth';
        varappend = '';
    ...
    case 'coastal'
        inputfile = [inputdir,...
            'SOCATv6_qrtrdeg_gridded_coast_monthly.nc'];
        timevarname = 'tmnth';
        varappend = '';
...
% Read info
latin=ncread([inputdir,inputfile],'ylat');
lonin=ncread([inputdir,inputfile],'xlon');
timeintemp=ncread([inputdir,inputfile], timevarname);

if strcmp (typegrid, 'coastal');
    tempin=ncread([inputdir,inputfile], ['coast_sst_ave_weighted', varappend]);
    salin=ncread([inputdir,inputfile], ['coast_salinity_ave_weighted', varappend]);
    fco2in=ncread([inputdir,inputfile], ['coast_fco2_ave_weighted', varappend]);
else
    tempin=ncread([inputdir,inputfile], ['sst_ave_weighted', varappend]);
    salin=ncread([inputdir,inputfile], ['salinity_ave_weighted', varappend]);
    fco2in=ncread([inputdir,inputfile], ['fco2_ave_weighted', varappend]);
end
