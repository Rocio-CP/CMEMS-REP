...
for v=1%:length(filestoadd)
  ...
  S.Attributes(2).Name='format_version'; S.Attributes(2).Value='1.4';
  ...
  S.Attributes(8).Name='Conventions'; S.Attributes(8).Value='CF-1.6 Copernicus-InSituTAC-FormatManual-1.4 Copernicus-InSituTAC-SRD-1.41 Copernicus-InSituTAC-ParametersList-3.2.0';
  ...
  S.Attributes(29).Name='update_interval'; S.Attributes(29).Value='P1Y';
  S.Attributes(31).Name='description'; S.Attributes(31).Value=['1 X 1 global mapped field of temperature from the GLODAPv2 data product. ',...
      'Mapping is performed using the DIVA software (Troupin et al., 2012). ',...
      'Error fields are calculated using the clever poor mans error calculation method in DIVA ',...
      '(Beckers et al., 2014). The error fields represent the mapping error only, ',...
      'and does not include measurement or calculation uncertainties in the input data.'];
  S.Attributes(32).Name='created'; S.Attributes(32).Value='Created by Siv K. Lauvset on 12-May-2016 18:47:05';
  S.Attributes(33).Name='title'; S.Attributes(33).Value='Global Ocean - Gridded In Situ reprocessed carbon observations - GLODAPv2.2016b';
  S.Attributes(30).Name='distribution_statement'; S.Attributes(30).Value='These data follow Copernicus standards; they are public and free of charge. User assumes all risk for use of data. User must display citation in any publication or product using data. User must contact PI prior to any commercial use of data.';
      S.Attributes(34).Name='netcdf_version'; S.Attributes(34).Value='netCDF-4';
