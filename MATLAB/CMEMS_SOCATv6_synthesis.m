...
% input folder
load([workrootdir,'04_2018/SOCATv6/workspace/SOCATv6.mat']);
load([workrootdir,'04_2018/ReferenceData/PlatformCodes.mat']);
%outdir=[workrootdir,'2018/SOCATv6/workspace/SOCATsplit_test/'];
outdir=['/Users/rpr061/Documents/localtestarea/CMEMS/',...
    'SOCAT-REP-OBSERVATIONS/'];


% From categorical to cellstr
% If done when creating SOCATv6.mat, the variables is >2GB
SOCATv6.Expocode=cellstr(SOCATv6.Expocode);
SOCATv6.ICEScode=cellstr(SOCATv6.ICEScode);
SOCATv6.PlatformName=cellstr(SOCATv6.PlatformName);

% Remap some of the ICES codes; some ships have 2 when it should be 1
SOCATv6.ICEScode(contains(SOCATv6.ICEScode,'06MT'))={'06M3'};
SOCATv6.ICEScode(contains(SOCATv6.ICEScode,'06P1'))={'06PO'};
SOCATv6.ICEScode(contains(SOCATv6.ICEScode,'35MV'))={'35MF'};

% In SOCAT tsv NaN are NaN
unique_expocodes=unique(SOCATv6.Expocode);
...
for ec=1:length(unique_expocodes)
    ...
    if any(ismember(ResearchVessels.SOCAT_code,...
        ...
        outputfile = ['VESSEL/GL_TS_TS_',platformcode,'_SOCATv6.nc'];
        if exist([outdir,outputfile], 'file')
            continue
        else
            expocodefilt=contains(SOCATv6.ICEScode,currenticescode);
        end
    ...
    elseif any(ismember(FixedMoorings.SOCAT_code,...
        ...
        outputfile = ['MOORING/GL_TS_MO_',platformcode,'_SOCATv6.nc'];
        if exist([outdir,outputfile], 'file')
            continue
        else
            expocodefilt=contains(SOCATv6.PlatformName,platformname);
        end
    ...
    elseif any(ismember(DriftingBuoys.SOCAT_code,...
        ...
        outputfile = ['GL_TS_DB_',platformcode,'_SOCATv6.nc'];
        if exist([outdir,outputfile], 'file')
            continue
        else
            expocodefilt=contains(SOCATv6.Expocode,currentexpocode);
        end
    ...
    latin = SOCATv6.latitudedecdegN(expocodefilt);
    lonin = SOCATv6.longitudedecdegE(expocodefilt);
    
    timeinsocat = datenum(double([SOCATv6.yr(expocodefilt),...
        SOCATv6.mon(expocodefilt),SOCATv6.day1(expocodefilt),...
        SOCATv6.hh(expocodefilt),SOCATv6.mm(expocodefilt),...
        SOCATv6.ss(expocodefilt)]));
    ...
    fco2in = round(SOCATv6.fCO2recuatm(expocodefilt)*1000.);
    fco2in(isnan(fco2in))=-9999;
    fco2inqc1 = SOCATv6.fCO2rec_flag(expocodefilt);
    fco2inqc=fco2inqc1;
    fco2inqc(fco2inqc1==2)=1;
    fco2indm = repmat('D',size(fco2in));
    
    tempin = round(SOCATv6.SSTdegC(expocodefilt)*1000.);
    tempin(isnan(tempin))=-9999;
    tempinqc = zeros(size(tempin));
    tempindm = repmat('D',size(tempin));
    
    salin = round(SOCATv6.sal(expocodefilt)*1000.);
    ...
    clearvars '-except' cmode ec workrootdir outdir SOCATv6 ...
