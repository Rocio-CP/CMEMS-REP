...
GLODAPv22020=load('/Users/rpr061/Downloads/GLODAPv2.2020_Merged_Master_File.mat');

%GLODAPv22019=load([workrootdir,'../original_files/GLODAPv2.2019_Merged_Master_File.mat']);

% Change all -9999 with NaN in GLODAP file
fn=fieldnames(GLODAPv22020);
for nfn=1:length(fn)
    if ~iscell(GLODAPv22020.(fn{nfn}))
        GLODAPv22020.(fn{nfn})(GLODAPv22020.(fn{nfn})==-9999)=NaN; end
end
% Some data points have NaN hour/minute
GLODAPv22020.G2hour(isnan(GLODAPv22020.G2hour))=0;
...
for f=1:length(fn)
    if ischar(GLODAPv22020_info.(fn{f})); GLODAPv22020_info.(fn{f})=cellstr(GLODAPv22020_info.(fn{f}));
    elseif isnumeric(GLODAPv22020_info.(fn{f})) & ~strcmp(fn{f},'CruiseCounter')
        GLODAPv22020_info.(fn{f})=cellstr(num2str(GLODAPv22020_info.(fn{f})));
    end
end
% Use the platform name as platform code if the callsign is missing
nocallsign=cellfun(@isempty,GLODAPv22020_info.CallSign_WMO);
allplatformcodes=GLODAPv22020_info.CallSign_WMO;
allplatformcodes(nocallsign)=GLODAPv22020_info.Name(nocallsign);
...
for nfv=1:length(flaggedvars)
    GLODAPv22020.(['G2',flaggedvars{nfv}])(GLODAPv22020.(['G2',flaggedvars{nfv}])==0)=8;
    GLODAPv22020.(['G2',flaggedvars{nfv}])(GLODAPv22020.(['G2',flaggedvars{nfv}])==1)=9;
    GLODAPv22020.(['G2',flaggedvars{nfv}])(GLODAPv22020.(['G2',flaggedvars{nfv}])==2)=1;
    GLODAPv22020.(['G2',flaggedvars{nfv}])(GLODAPv22020.(['G2',flaggedvars{nfv}])==3)=2;
    GLODAPv22020.(['G2',flaggedvars{nfv}])(GLODAPv22020.(['G2',flaggedvars{nfv}])==4)=4;
    GLODAPv22020.(['G2',flaggedvars{nfv}])(GLODAPv22020.(['G2',flaggedvars{nfv}])==5)=9;
    GLODAPv22020.(['G2',flaggedvars{nfv}])(GLODAPv22020.(['G2',flaggedvars{nfv}])==6)=8;
    GLODAPv22020.(['G2',flaggedvars{nfv}])(GLODAPv22020.(['G2',flaggedvars{nfv}])==7)=0;
    GLODAPv22020.(['G2',flaggedvars{nfv}])(GLODAPv22020.(['G2',flaggedvars{nfv}])==8)=0;
    GLODAPv22020.(['G2',flaggedvars{nfv}])(GLODAPv22020.(['G2',flaggedvars{nfv}])==9)=9;
    GLODAPv22020.(['G2',flaggedvars{nfv}])=int8(GLODAPv22020.(['G2',flaggedvars{nfv}]));
end

% Temperature and bottom depth are assumed good if they exist
GLODAPv22020.G2bottomdepthf=zeros(size(GLODAPv22020.G2salinityf));
GLODAPv22020.G2temperaturef=ones(size(GLODAPv22020.G2salinityf));
...
%for platnum=1:length(unique_platform_code)
    ...
    filterdata=ismember(GLODAPv22020.G2cruise,GLODAPv22020_info.CruiseCounter(filterec));
...
%     
    ...
    switch source_platform_category_code
        case '31'
            ...
            outputfile = ['VESSEL/GL_PR_BO_',platform_code,'-GLODAPv22020.nc'];
        case '21'
            source='propelled manned submersible';
            outputfile = ['ETC/GL_PR_BO_',platform_code,'-GLODAPv22020.nc'];
        case '62'
            source='aeroplane';
            outputfile = ['ETC/GL_PR_BO_',platform_code,'-GLODAPv22020.nc'];
    end
    ...
    depthinglodap=GLODAPv22020.G2depth(filterdata); % data only from the relevant cruises
    [daa,dbb,dcc]=unique(depthinglodap); % string of unique depth values
    
    timeinglodapMATLAB = datenum(([GLODAPv22020.G2year(filterdata),...
        GLODAPv22020.G2month(filterdata),GLODAPv22020.G2day(filterdata),...
        GLODAPv22020.G2hour(filterdata),GLODAPv22020.G2minute(filterdata),...
        zeros(size(GLODAPv22020.G2minute(filterdata)))]));
    ...
    latinglodap = GLODAPv22020.G2latitude(filterdata);
    latin=latinglodap(tbb);
    loninglodap=GLODAPv22020.G2longitude(filterdata);
    lonin=loninglodap(tbb);
    
    % Variables
    for vv=1:length(invars)
        % Data from the relevant cruises
        eval([invars{vv},'inglodap=round(GLODAPv22020.',['G2',invars{vv}],'(filterdata)*1000.);']);
        eval([invars{vv},'finglodap=GLODAPv22020.',['G2',invars{vv}],'f(filterdata);']);
...
%     % Define _FillValue (does not work after reDef);
  ...
  %              eval(['netcdf.delAtt(ncid,',invars{vv},',''_FillValue'');']);
    ...
    netcdf.putAtt(ncid, globid, 'title', 'Global Ocean - In Situ reprocessed carbon observations - GLODAPv2.2020');
...
%end
    ...
    netcdf.putAtt(ncid, globid, 'update_interval', 'P1Y');
        ...
        'GLODAPv2.2020 is described in Olsen et al. (2020)a, and Olsen et al. (2020)b; traceable citations are essential for justifying and sustaining the effort.']);
    ...
    netcdf.putAtt(ncid, globid, 'doi', 'https://doi.org/10.25921/2c8h-sa89 https://doi.org/10.5194/essd‑2020‑165.');
    ...
    % Clear variables
    clearvars '-except' workrootdir outdir GLODAPv22020 GLODAPv22020_info ...
