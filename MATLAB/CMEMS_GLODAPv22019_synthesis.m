...
for platnum=106:length(unique_platform_code)
    ...
    pi_institution=strjoin(transpose(unique(GLODAPv22019_info.PI_Institution(filterec))),'/');
    pi_institution_edmo_code=regexprep(char(join(transpose(unique(GLODAPv22019_info.PI_EDMO(filterec))))),' +',' ');
    pis=strjoin(transpose(unique(GLODAPv22019_info.CarbonPI(filterec))),'/');
...
if (strcmp(source_platform_category_code,'31') | strcmp(source_platform_category_code,'32')) & ~isempty(ices_platform_code)
    ...
    netcdf.putAtt(ncid, globid, 'citation', ['These data were collected and made freely available by the Copernicus project and the programs that contribute to it. ',...
        'GLODAPv2.2019 is described in Olsen et al. (2019)a, and Olsen et al. (2019)b; traceable citations are essential for justifying and sustaining the effort.']);
